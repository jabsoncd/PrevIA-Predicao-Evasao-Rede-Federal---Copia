...
class DataIngestionConfig:
    ...
    test_data_path = os.path.join('artifacts', 'X_train2.csv')
    raw_data_path = os.path.join('artifacts', 'base_redeFederal_2022_tecnico_regiaoMetropolitana.csv')
...
class DataIngestion:
    ...
    def apply_data_ingestion(self):
        ...
        try:
            ...
            df.rename(columns={'categoria_situacao': 'evadido'}, inplace=True)
            df['evadido'] = df['evadido'].map({'Evadidos': 1, 'Em curso': 0, 'Concluintes': 0})

            X = df.drop(columns=['evadido'])
            y = df['evadido'].copy()
